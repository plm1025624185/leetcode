...
//    }
    ...
    public List<String> letterCombinations(String digits) {
        ...
        List<String> list = new ArrayList<>();
        _letterCombinations(list, "", array, 0);
        return list;
    }

    private void _letterCombinations(List<String> list, String str, char[] array, int index) {
        if (index == array.length) {list.add(str); return;}
        String key = keys[array[index] - '0'];
        char[] keyArray = key.toCharArray();
        for (int i = 0; i < keyArray.length; i++) {
            _letterCombinations(list, str + keyArray[i], array, index + 1);
        }
    }
}
