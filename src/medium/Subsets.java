...
//    }
    ...
    public List<List<Integer>> subsets(int[] nums) {
        ...
        _subsets(nums, result, new ArrayList<Integer>(), 0);
        return result;
    }

    private void _subsets(int[] nums, List<List<Integer>> result, List<Integer> list, int start) {
        if (start > nums.length) return;
        for (int i = start; i < nums.length; i++) {
            list.add(nums[i]);
            result.add(new ArrayList<>(list));
            _subsets(nums, result, list, i + 1);
            list.remove(list.size() - 1);
        }
    }
}
